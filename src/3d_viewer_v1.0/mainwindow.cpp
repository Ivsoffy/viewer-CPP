...
void MainWindow::open_file() {
  ...
  int error = object_to_object_data(
      ui->lineEdit_file_input->text().toLocal8Bit().data(), &data);
  ...
  if (error) {
    ...
    ui->openGLWidget->vertex_array = (struct vertex *)calloc(sizeof(struct vertex *), data.vertex_count);

    object_data_for_opengl(data, ui->openGLWidget->vertex_array,
                           &ui->openGLWidget->polygons_array,
                           &ui->openGLWidget->edges_counter,
