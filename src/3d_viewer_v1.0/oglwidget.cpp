...
////      gl_model glModel(vertex_array, polygons_array, edges_counter, total_edges);
////      dto_buff *dto_b = glModel.getBuffer();
...
//      glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(polygons_array), polygons_array, GL_STATIC_DRAW);

//      glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer);
//      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBuffer);

//      glEnableClientState(GL_VERTEX_ARRAY);
//       glVertexPointer(3, GL_FLOAT, sizeof(double)*3, BUFFER_OFFSET(0));    // The starting point of the VBO, for the vertices
...
//  glBufferData(GL_ARRAY_BUFFER, sizeof(MyVertex)*8, &pvertex[0].x, GL_STATIC_DRAW);
...
//  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(ushort)*32, pindices, GL_STATIC_DRAW);
...
//  glDrawElements(GL_LINES, 32, GL_UNSIGNED_SHORT, 0);    // The starting point of the IBO
////  uint pol_ar_shift = 0;
////  for (uint i = 0; i < polygon_count; i++) {
...
void OGLwidget::paintGL() {
      ...
      GLmodel *glm = new GLmodel;

      glm->FillBufferData(glm->cubeVertices, glm->cubeIndices);
      VertexBuffer_ = glm->GetVertexBuffer();
      IndexBuffer_ = glm->GetIndexBuffer();
      IndecesSize_ = glm->GetIndicesSize();
      VerticesSize_ = glm->GetVerticesSize();
